var channels = ["cretetion", "ESL_SC2", "freecodecamp", "habathcx", "noobs2ninjas", "OgamingSC2", "RobotCaleb", "storbeck", "brunofin", "comster404"];
var selected = "all";
var search = "";
      ...
      if(users.error !== undefined){
        ...
        $.getJSON("https://wind-bow.glitch.me/twitch-api/streams/" + channels[each], function(users){
          if(users.stream === null){
            $(".user" + each).html(channels[each] + "<i class='fa fa-circle offline float-right' title='Offline' aria-hidden='true'></i>");
            $(".c" + each).addClass("off");
            $(".c" + each).addClass("all");
          }
          else {
...
function filterResults(search, selected){
  for(each in channels){
    if($(".user" + each).text().toLowerCase().match(search) !== null && $(".c" + each).hasClass(selected)){
      $(".c" + each).show();
    }
    else {
      $(".c" + each).hide();
